...
import { Chapter, Note, NoteChapter } from "@prisma/client";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogContent,
...
interface ChaptersFormProps {
  initialData: Note & { notechapters: NoteChapter[] };
  noteId: string;
}
...
export const ChaptersForm = ({ initialData, noteId }: ChaptersFormProps) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      await axios.post(`/api/notes/${noteId}/chapters`, values);
      router.refresh();
  ...
  const onReorder = async (updateData: { id: string; position: number }[]) => {
    try {
      setIsUpdating(true);
      await axios.put(`/api/notes/${noteId}/chapters/reorder`, {
        list: updateData,
      });
  ...
  const onEdit = (id: string) => {
    router.push(`/tutor/notes/${noteId}/chapters/${id}`);
  };
    ...
    <div className="relative mt-6 bg-[#020817] rounded-md p-4 border">
      ...
      {!isCreating && (
        <div
          className={cn(
            "text-sm mt-2",
            !initialData.notechapters.length && "text-slate-500 italic"
          )}
        >
          {!initialData.notechapters.length && "No chapters"}
          <ChaptersList
            onEdit={onEdit}
            onReorder={onReorder}
            items={initialData.notechapters || []}
          />
        </div>
      )}
      {initialData.notechapters.length !== 0 && !isCreating && (
