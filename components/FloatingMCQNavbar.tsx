...
import { FloatingDock } from "@/app/(dashboard)/(routes)/predictor/_components/floating-dock";

const FloatingMCQNavbar = () => {
  ...
  return (
    <div className="fixed bottom-10 left-1/2 transform -translate-x-1/2 p-2 bg-slate-800 text-white shadow-md z-50 rounded-full w-[95%] max-w-[800px] mb-0 md-contentfitter">
      <nav className="flex items-center space-x-2 overflow-auto">
        {/* Home Icon Link */}
        <Link href="/mcq-solver" className="flex items-center">
        ...
        {pathSegments?.map((segment, index) => {
          ...
          if (segment === "mcq-solver") return null;
...
export default FloatingMCQNavbar;
